...
export class InappropriateContentService {
  constructor(
    @InjectRepository(InappropriateContent)
    private inappropriateContentRepository: Repository<InappropriateContent>,
  ) {}

  async createInappropriateContent(
    inappropriateContent: CreateInappropriateContentDto,
  ) {
    const inappropriateContentFound =
      await this.inappropriateContentRepository.findOne({
        where: {
    ...
    if (inappropriateContentFound) {
      return new HttpException(
        'Inappropriate Content already exists',
        HttpStatus.CONFLICT,
      );
    }
    const newInappropriateContent =
      this.inappropriateContentRepository.create(inappropriateContent);
    return {
      message: 'success',
      data: await this.inappropriateContentRepository.save(
        newInappropriateContent,
      ),
    };
  }
  
  async createInappropriateContents(
    inappropriateContents: CreateInappropriateContentDto[],
  ) {
  ...
  async getInappropriateContents() {
    return {
      message: 'success',
      data: await this.inappropriateContentRepository.find(),
    };
  }

  async getInappropriateContent(id: number) {
    const inappropriateContentFound =
      await this.inappropriateContentRepository.findOne({ where: { id } });
    if (!inappropriateContentFound) {
      return new HttpException(
        'Inappropriate Content not found',
  ...
  async updateInappropriateContent(
    id: number,
    inappropriateContent: UpdateInappropriateContentDto,
  ) {
    const inappropriateContentFound =
      await this.inappropriateContentRepository.findOne({ where: { id } });
    if (!inappropriateContentFound) {
    ...
    const updateInappropriateContent = Object.assign(
      inappropriateContentFound,
